...
#        ]
    ...
    for server_scan_result in scanner.get_results():
        ...
        ss2_attempt = server_scan_result.scan_result.ssl_2_0_cipher_suites
        if ss2_attempt.status == ScanCommandAttemptStatusEnum.ERROR:
            # An error happened when this scan command was run
            _print_failed_scan_command_attempt(ss2_attempt)
        elif ss2_attempt.status == ScanCommandAttemptStatusEnum.COMPLETED:
            # This scan command was run successfully
            ssl2_result = ss2_attempt.result
            assert ssl2_result
            print("\nAccepted cipher suites for SSL 2.0:")
            for accepted_cipher_suite in ssl2_result.accepted_cipher_suites:
                print(f"* {accepted_cipher_suite.cipher_suite.name}")

        # Process the result of the SSL 1.2 scan command
        ssl2_attempt = server_scan_result.scan_result.tls_1_2_cipher_suites
